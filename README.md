...
4. **첫 실행 (수동)**
   ...
   - **첫 실행에서는 Slack 알림이 오지 않는 게 정상** 현재 최신 글 번호를 기준점으로만 저장하기 때문입니다 (과거 글이 한꺼번에 알림 오는 것을 막기 위함).
   - `state/last_article_id.txt` 파일이 생기고 커밋되는지 확인.

5. **이후부터는 자동**
   - 5분마다 자동 실행되어, 기준점보다 새 글이 있으면 Slack으로 전송하고 기준점을 갱신
   (현재는 5분이나 문제 발생 시 10분으로 변경 필요)
